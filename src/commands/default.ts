...
export const defaultCommand: CommandModule = {
  command: "$0",
  describe: "Install Enterprise Readiness Protocol skills to your coding agents",
  builder: (yargs) => yargs.option("agent", { type: "array", string: true, describe: "Target specific agents" }),
  handler: async (argv) => {
    console.log();
    console.log(chalk.bold("  Interf") + " — Enterprise Readiness Protocol");
    console.log();
    ...
    console.log(chalk.bold("  draft a readiness contract and preview rollout for <company>"));
    console.log();
